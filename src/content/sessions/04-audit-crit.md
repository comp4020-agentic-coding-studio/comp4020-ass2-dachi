---
title: Audit crit
description:
  A crit session for doors you're unsure how to judge, held on the day the
  door audit is due
week: 4
date: 2027-03-15
teachers:
  - petra-voss
spec:
  - you bring at least one door you couldn't confidently judge on your own
  - you can state the criterion you're applying to it before the room weighs
    in
  - you leave with a decision on that door, made before you submit
related: []
---

## Before the session

Finish your ten-door survey to the point where nine are settled and bring the
tenth — the one where you genuinely don't know whether the door works or
fails.

## In the session

This is a crit, not a lecture: each person states their uncertain door, the
criterion they're judging it against, and the room argues it out. The
audit's own brief warns against picking ten doors to make a point rather than
to test one — this session is the last chance to catch that before marking.

## Afterwards

The door audit is submitted by the end of this session, worth 20% of the
course.
